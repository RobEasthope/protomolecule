...
export function parseSemver(version: string): SemverParts {
  ...
  if (Number.isNaN(major) || Number.isNaN(minor) || Number.isNaN(patch)) {
    throw new Error(`Invalid semver components in: ${version} (major=${major}, minor=${minor}, patch=${patch})`);
  }

  // Validate non-negative
  if (major < 0 || minor < 0 || patch < 0) {
    throw new Error(`Invalid semver: negative values not allowed in ${version}`);
  }
...
function determineBumpType(packages: Package[]): BumpType {
  ...
  for (const pkg of packages) {
    ...
    if (prevTag) {
      ...
      console.log(`First release of ${name}@${version} (no previous tag found)`);
      if (bumpType === 'patch') {
        bumpType = 'minor';
      }
    }
 ...
 * Pure function - easily testable
 */
export function calculateNewVersion(currentVersion: string, bumpType: BumpType): string {
...
function remoteTagExists(tag: string): boolean {
  try {
    const output = execSync('git ls-remote --tags origin', { encoding: 'utf8' });
...
function bumpMonorepo(): void {
  ...
  try {
    ...
    if (remoteTagExists(tag)) {
      console.log(`⚠️  Tag ${tag} already exists on remote (concurrent creation detected)`);
    } else {
