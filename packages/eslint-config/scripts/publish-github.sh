...
echo "📦 Publishing @protomolecule/eslint-config to GitHub Packages"
echo "========================================================="
...
if npm whoami --registry=https://npm.pkg.github.com --userconfig ~/.npmrc.github-eslint > /dev/null 2>&1; then
  echo "✅ Authenticated as: $(npm whoami --registry=https://npm.pkg.github.com --userconfig ~/.npmrc.github-eslint)"
else
  echo "❌ Authentication failed"
  rm ~/.npmrc.github-eslint
  exit 1
fi
...
npm publish --userconfig ~/.npmrc.github-eslint --access public < /dev/null 2>&1 | tee /tmp/npm-publish-eslint.log
EXIT_CODE=${PIPESTATUS[0]}

# Read output for error checking
OUTPUT=$(cat /tmp/npm-publish-eslint.log 2>/dev/null || echo "")
rm -f /tmp/npm-publish-eslint.log
